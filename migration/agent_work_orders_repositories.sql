...
-- =====================================================
-- This migration creates the cortex_configured_repositories table
...
-- Create cortex_configured_repositories table
CREATE TABLE IF NOT EXISTS cortex_configured_repositories (
...
CREATE INDEX IF NOT EXISTS idx_configured_repositories_commands
    ON cortex_configured_repositories USING GIN(default_commands);
...
CREATE TRIGGER update_configured_repositories_updated_at
    BEFORE UPDATE ON cortex_configured_repositories
    FOR EACH ROW
...
ALTER TABLE cortex_configured_repositories ENABLE ROW LEVEL SECURITY;

-- Policy 1: Service role has full access (for API operations)
CREATE POLICY "Allow service role full access to cortex_configured_repositories"
    ON cortex_configured_repositories
    FOR ALL
    USING (auth.role() = 'service_role');

-- Policy 2: Authenticated users can read and update (for frontend operations)
CREATE POLICY "Allow authenticated users to read and update cortex_configured_repositories"
    ON cortex_configured_repositories
    FOR ALL
    TO authenticated
    USING (true);
...
COMMENT ON TABLE cortex_configured_repositories IS
...
COMMENT ON COLUMN cortex_configured_repositories.id IS
    'Unique UUID identifier for the configured repository';

COMMENT ON COLUMN cortex_configured_repositories.repository_url IS
    'GitHub repository URL (must be https://github.com/owner/repo format)';

COMMENT ON COLUMN cortex_configured_repositories.display_name IS
    'Human-readable repository name extracted from GitHub API (e.g., "owner/repo-name")';

COMMENT ON COLUMN cortex_configured_repositories.owner IS
    'Repository owner/organization name extracted from GitHub API';

COMMENT ON COLUMN cortex_configured_repositories.default_branch IS
    'Default branch name extracted from GitHub API (typically "main" or "master")';

COMMENT ON COLUMN cortex_configured_repositories.is_verified IS
    'Boolean flag indicating if repository access has been verified via GitHub API';

COMMENT ON COLUMN cortex_configured_repositories.last_verified_at IS
    'Timestamp of last successful repository verification';

COMMENT ON COLUMN cortex_configured_repositories.default_sandbox_type IS
...
COMMENT ON COLUMN cortex_configured_repositories.default_commands IS
    'JSONB array of default workflow commands for work orders (e.g., ["create-branch", "planning", "execute", "commit", "create-pr"])';

COMMENT ON COLUMN cortex_configured_repositories.created_at IS
    'Timestamp when repository configuration was created';

COMMENT ON COLUMN cortex_configured_repositories.updated_at IS
...
BEGIN
    IF EXISTS (
        ...
        AND table_name = 'cortex_configured_repositories'
    ) THEN
        RAISE NOTICE '✓ Table cortex_configured_repositories created successfully';
    ELSE
        RAISE EXCEPTION '✗ Table cortex_configured_repositories was not created';
    END IF;
END $$;
...
BEGIN
    IF (
        ...
        WHERE tablename = 'cortex_configured_repositories'
    ) >= 4 THEN
...
BEGIN
    IF EXISTS (
        ...
        WHERE tgrelid = 'cortex_configured_repositories'::regclass
...
DROP TABLE IF EXISTS cortex_configured_repositories CASCADE;
...
-- MIGRATION COMPLETE
-- =====================================================
-- The cortex_configured_repositories table is now ready for use
-- Next steps:
