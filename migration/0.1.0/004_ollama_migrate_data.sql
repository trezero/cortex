...
BEGIN
    ...
    WHERE table_name = 'cortex_crawled_pages'
    ...
    IF crawled_pages_count > 0 THEN
        ...
        -- Drop old column
        ALTER TABLE cortex_crawled_pages DROP COLUMN IF EXISTS embedding;
    END IF;

    -- Same for code_examples
    SELECT COUNT(*) INTO code_examples_count
    FROM information_schema.columns
    WHERE table_name = 'cortex_code_examples'
    AND column_name = 'embedding';
    ...
    IF code_examples_count > 0 THEN
        ...
        ALTER TABLE cortex_code_examples DROP COLUMN IF EXISTS embedding;
    END IF;
END $$;

-- Drop old indexes if they exist
DROP INDEX IF EXISTS idx_cortex_crawled_pages_embedding;
DROP INDEX IF EXISTS idx_cortex_code_examples_embedding;
