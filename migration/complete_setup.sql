-- =====================================================
-- Cortex Complete Database Setup
-- =====================================================
...
-- the complete Cortex database schema and initial data
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_cortex_settings_key ON cortex_settings(key);
CREATE INDEX IF NOT EXISTS idx_cortex_settings_category ON cortex_settings(category);
...
CREATE TRIGGER update_cortex_settings_updated_at
    BEFORE UPDATE ON cortex_settings
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- Create RLS (Row Level Security) policies for settings
ALTER TABLE cortex_settings ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow service role full access" ON cortex_settings
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow authenticated users to read and update" ON cortex_settings
...
INSERT INTO cortex_settings (key, value, is_encrypted, category, description) VALUES
...
INSERT INTO cortex_settings (key, encrypted_value, is_encrypted, category, description) VALUES
...
COMMENT ON TABLE cortex_settings IS 'Stores application configuration including API keys, RAG settings, and code extraction parameters';
...
CREATE INDEX IF NOT EXISTS idx_cortex_sources_title ON cortex_sources(title);
...
CREATE INDEX IF NOT EXISTS idx_cortex_sources_metadata ON cortex_sources USING GIN(metadata);
CREATE INDEX IF NOT EXISTS idx_cortex_sources_knowledge_type ON cortex_sources((metadata->>'knowledge_type'));

-- Add comments to document the columns
COMMENT ON COLUMN cortex_sources.source_id IS 'Unique hash identifier for the source (16-char SHA256 hash of URL)';
...
COMMENT ON COLUMN cortex_sources.title IS 'Descriptive title for the source (e.g., "Pydantic AI API Reference")';
COMMENT ON COLUMN cortex_sources.metadata IS 'JSONB field storing knowledge_type, tags, and other metadata';

-- Create the documentation chunks table
CREATE TABLE IF NOT EXISTS cortex_crawled_pages (
    ...
    FOREIGN KEY (source_id) REFERENCES cortex_sources(source_id) ON DELETE CASCADE
);

-- Multi-dimensional indexes
CREATE INDEX IF NOT EXISTS idx_cortex_crawled_pages_embedding_384 ON cortex_crawled_pages USING ivfflat (embedding_384 vector_cosine_ops) WITH (lists = 100);
CREATE INDEX IF NOT EXISTS idx_cortex_crawled_pages_embedding_768 ON cortex_crawled_pages USING ivfflat (embedding_768 vector_cosine_ops) WITH (lists = 100);
CREATE INDEX IF NOT EXISTS idx_cortex_crawled_pages_embedding_1024 ON cortex_crawled_pages USING ivfflat (embedding_1024 vector_cosine_ops) WITH (lists = 100);
CREATE INDEX IF NOT EXISTS idx_cortex_crawled_pages_embedding_1536 ON cortex_crawled_pages USING ivfflat (embedding_1536 vector_cosine_ops) WITH (lists = 100);
...
-- Other indexes for cortex_crawled_pages
CREATE INDEX idx_cortex_crawled_pages_metadata ON cortex_crawled_pages USING GIN (metadata);
CREATE INDEX idx_cortex_crawled_pages_source_id ON cortex_crawled_pages (source_id);
-- Hybrid search indexes
CREATE INDEX idx_cortex_crawled_pages_content_search ON cortex_crawled_pages USING GIN (content_search_vector);
CREATE INDEX idx_cortex_crawled_pages_content_trgm ON cortex_crawled_pages USING GIN (content gin_trgm_ops);
-- Multi-dimensional embedding indexes
CREATE INDEX idx_cortex_crawled_pages_embedding_model ON cortex_crawled_pages (embedding_model);
CREATE INDEX idx_cortex_crawled_pages_embedding_dimension ON cortex_crawled_pages (embedding_dimension);
CREATE INDEX idx_cortex_crawled_pages_llm_chat_model ON cortex_crawled_pages (llm_chat_model);

-- Create the code_examples table
CREATE TABLE IF NOT EXISTS cortex_code_examples (
    id BIGSERIAL PRIMARY KEY,
    url VARCHAR NOT NULL,
    chunk_number INTEGER NOT NULL,
...
-- Enable RLS on cortex_page_metadata
ALTER TABLE cortex_page_metadata ENABLE ROW LEVEL SECURITY;

-- Multi-dimensional indexes
CREATE INDEX IF NOT EXISTS idx_cortex_code_examples_embedding_384 ON cortex_code_examples USING ivfflat (embedding_384 vector_cosine_ops) WITH (lists = 100);
CREATE INDEX IF NOT EXISTS idx_cortex_code_examples_embedding_768 ON cortex_code_examples USING ivfflat (embedding_768 vector_cosine_ops) WITH (lists = 100);
CREATE INDEX IF NOT EXISTS idx_cortex_code_examples_embedding_1024 ON cortex_code_examples USING ivfflat (embedding_1024 vector_cosine_ops) WITH (lists = 100);
CREATE INDEX IF NOT EXISTS idx_cortex_code_examples_embedding_1536 ON cortex_code_examples USING ivfflat (embedding_1536 vector_cosine_ops) WITH (lists = 100);
-- Note: 3072-dimensional embeddings cannot have vector indexes due to PostgreSQL vector extension 2000 dimension limit
-- The embedding_3072 column exists but cannot be indexed with current pgvector version

-- Other indexes for cortex_code_examples
CREATE INDEX idx_cortex_code_examples_metadata ON cortex_code_examples USING GIN (metadata);
CREATE INDEX idx_cortex_code_examples_source_id ON cortex_code_examples (source_id);
-- Hybrid search indexes
CREATE INDEX idx_cortex_code_examples_content_search ON cortex_code_examples USING GIN (content_search_vector);
CREATE INDEX idx_cortex_code_examples_content_trgm ON cortex_code_examples USING GIN (content gin_trgm_ops);
CREATE INDEX idx_cortex_code_examples_summary_trgm ON cortex_code_examples USING GIN (summary gin_trgm_ops);
-- Multi-dimensional embedding indexes
CREATE INDEX idx_cortex_code_examples_embedding_model ON cortex_code_examples (embedding_model);
CREATE INDEX idx_cortex_code_examples_embedding_dimension ON cortex_code_examples (embedding_dimension);
CREATE INDEX idx_cortex_code_examples_llm_chat_model ON cortex_code_examples (llm_chat_model);
...
ALTER TABLE cortex_crawled_pages ENABLE ROW LEVEL SECURITY;
ALTER TABLE cortex_sources ENABLE ROW LEVEL SECURITY;
ALTER TABLE cortex_code_examples ENABLE ROW LEVEL SECURITY;

-- Create policies that allow anyone to read
CREATE POLICY "Allow public read access to cortex_crawled_pages"
  ON cortex_crawled_pages
  FOR SELECT
  TO public
  USING (true);

CREATE POLICY "Allow public read access to cortex_sources"
  ON cortex_sources
  FOR SELECT
  TO public
  USING (true);

CREATE POLICY "Allow public read access to cortex_code_examples"
  ON cortex_code_examples
  FOR SELECT
  TO public
  USING (true);

CREATE POLICY "Allow public read access to cortex_page_metadata"
  ON cortex_page_metadata
  FOR SELECT
  TO public
  USING (true);
...
CREATE TABLE IF NOT EXISTS cortex_projects (
  ...
  parent_project_id UUID REFERENCES cortex_projects(id) ON DELETE SET NULL,
  metadata JSONB DEFAULT '{}',
  tags TEXT[] DEFAULT '{}',
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
...
CREATE TABLE IF NOT EXISTS cortex_tasks (
  ...
  parent_task_id UUID REFERENCES cortex_tasks(id) ON DELETE CASCADE,
  title TEXT NOT NULL,
  description TEXT DEFAULT '',
...
CREATE TABLE IF NOT EXISTS cortex_project_sources (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID REFERENCES cortex_projects(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS cortex_document_versions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID REFERENCES cortex_projects(id) ON DELETE CASCADE,
  task_id UUID REFERENCES cortex_tasks(id) ON DELETE CASCADE, -- DEPRECATED: No longer used, kept for historical data
...
CREATE INDEX IF NOT EXISTS idx_cortex_tasks_project_id ON cortex_tasks(project_id);
CREATE INDEX IF NOT EXISTS idx_cortex_tasks_status ON cortex_tasks(status);
CREATE INDEX IF NOT EXISTS idx_cortex_tasks_assignee ON cortex_tasks(assignee);
CREATE INDEX IF NOT EXISTS idx_cortex_tasks_order ON cortex_tasks(task_order);
CREATE INDEX IF NOT EXISTS idx_cortex_tasks_priority ON cortex_tasks(priority);
CREATE INDEX IF NOT EXISTS idx_cortex_tasks_archived ON cortex_tasks(archived);
CREATE INDEX IF NOT EXISTS idx_cortex_tasks_archived_at ON cortex_tasks(archived_at);
CREATE INDEX IF NOT EXISTS idx_cortex_project_sources_project_id ON cortex_project_sources(project_id);
CREATE INDEX IF NOT EXISTS idx_cortex_project_sources_source_id ON cortex_project_sources(source_id);
...
CREATE INDEX IF NOT EXISTS idx_cortex_document_versions_project_id ON cortex_document_versions(project_id);
CREATE INDEX IF NOT EXISTS idx_cortex_document_versions_task_id ON cortex_document_versions(task_id);
CREATE INDEX IF NOT EXISTS idx_cortex_document_versions_field_name ON cortex_document_versions(field_name);
CREATE INDEX IF NOT EXISTS idx_cortex_document_versions_version_number ON cortex_document_versions(version_number);
CREATE INDEX IF NOT EXISTS idx_cortex_document_versions_created_at ON cortex_document_versions(created_at);

-- Apply triggers to tables
CREATE OR REPLACE TRIGGER update_cortex_projects_updated_at
    BEFORE UPDATE ON cortex_projects
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE OR REPLACE TRIGGER update_cortex_tasks_updated_at
    BEFORE UPDATE ON cortex_tasks
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
COMMENT ON COLUMN cortex_tasks.assignee IS 'The agent or user assigned to this task. Can be any valid agent name or "User"';
...
COMMENT ON COLUMN cortex_tasks.archived IS 'Soft delete flag - TRUE if task is archived/deleted';
COMMENT ON COLUMN cortex_tasks.archived_at IS 'Timestamp when task was archived';
COMMENT ON COLUMN cortex_tasks.archived_by IS 'User/system that archived the task';

-- Add comments for versioning table
COMMENT ON TABLE cortex_document_versions IS 'Version control for JSONB fields in projects only - task versioning has been removed to simplify MCP operations';
COMMENT ON COLUMN cortex_document_versions.field_name IS 'Name of JSONB field being versioned (docs, features, data) - task fields and prd removed as unused';
COMMENT ON COLUMN cortex_document_versions.content IS 'Full snapshot of field content at this version';
COMMENT ON COLUMN cortex_document_versions.change_type IS 'Type of change: create, update, delete, restore, backup';
COMMENT ON COLUMN cortex_document_versions.document_id IS 'For docs arrays, the specific document ID that was changed';
COMMENT ON COLUMN cortex_document_versions.task_id IS 'DEPRECATED: No longer used for new versions, kept for historical task version data';

-- =====================================================
-- SECTION 7: MIGRATION TRACKING
-- =====================================================

-- Create cortex_migrations table for tracking applied database migrations
...
CREATE INDEX IF NOT EXISTS idx_cortex_prompts_name ON cortex_prompts(prompt_name);

-- Add trigger to automatically update updated_at timestamp
CREATE OR REPLACE TRIGGER update_cortex_prompts_updated_at
    BEFORE UPDATE ON cortex_prompts
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
ALTER TABLE cortex_projects ENABLE ROW LEVEL SECURITY;
ALTER TABLE cortex_tasks ENABLE ROW LEVEL SECURITY;
ALTER TABLE cortex_project_sources ENABLE ROW LEVEL SECURITY;
ALTER TABLE cortex_document_versions ENABLE ROW LEVEL SECURITY;
ALTER TABLE cortex_prompts ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for service role (full access)
CREATE POLICY "Allow service role full access to cortex_projects" ON cortex_projects
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow service role full access to cortex_tasks" ON cortex_tasks
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow service role full access to cortex_project_sources" ON cortex_project_sources
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow service role full access to cortex_document_versions" ON cortex_document_versions
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow service role full access to cortex_prompts" ON cortex_prompts
    FOR ALL USING (auth.role() = 'service_role');

-- Create RLS policies for authenticated users
CREATE POLICY "Allow authenticated users to read and update cortex_projects" ON cortex_projects
    FOR ALL TO authenticated
    USING (true);

CREATE POLICY "Allow authenticated users to read and update cortex_tasks" ON cortex_tasks
    FOR ALL TO authenticated
    USING (true);

CREATE POLICY "Allow authenticated users to read and update cortex_project_sources" ON cortex_project_sources
    FOR ALL TO authenticated
    USING (true);

CREATE POLICY "Allow authenticated users to read cortex_document_versions" ON cortex_document_versions
    FOR SELECT TO authenticated
    USING (true);

CREATE POLICY "Allow authenticated users to read cortex_prompts" ON cortex_prompts
    FOR SELECT TO authenticated
    USING (true);
...
INSERT INTO cortex_prompts (prompt_name, prompt, description) VALUES
...
-- cortex_project_system_registrations: Tracks which systems have synced with which projects
...
CREATE INDEX IF NOT EXISTS idx_project_system_reg_project ON cortex_project_system_registrations(project_id);
CREATE INDEX IF NOT EXISTS idx_project_system_reg_system  ON cortex_project_system_registrations(system_id);
...
-- SETUP COMPLETE
-- =====================================================
-- Your Cortex database is now fully configured!
--
-- Next steps:
