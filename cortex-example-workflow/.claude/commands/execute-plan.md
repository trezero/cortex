---
description: Execute a development plan with full Cortex task management integration
argument-hint: [plan-file-path]
---

# Execute Development Plan with Cortex Task Management

You are about to execute a comprehensive development plan with integrated Cortex task management. This workflow ensures systematic task tracking and implementation throughout the entire development process.

## Critical Requirements

**MANDATORY**: Throughout the ENTIRE execution of this plan, you MUST maintain continuous usage of Cortex for task management. DO NOT drop or skip Cortex integration at any point. Every task from the plan must be tracked in Cortex from creation to completion.
...
## Step 2: Project Setup in Cortex

1. Check if a project ID is specified in CLAUDE.md for this feature
   - Look for any Cortex project references in CLAUDE.md
   - If found, use that project ID

2. If no project exists:
   - Create a new project in Cortex using `mcp__cortex__manage_project`
   - Use a descriptive title based on the plan's objectives
   - Store the project ID for use throughout execution

## Step 3: Create All Tasks in Cortex

For EACH task identified in the plan:
1. Create a corresponding task in Cortex using `mcp__cortex__manage_task("create", ...)`
...
**IMPORTANT**: Create ALL tasks in Cortex upfront before starting implementation. This ensures complete visibility of the work scope.
...
### 5.1 Start Task
- Move the current task to "doing" status in Cortex: `mcp__cortex__manage_task("update", task_id=..., status="doing")`
...
### 5.3 Complete Task
- Once implementation is complete, move task to "review" status: `mcp__cortex__manage_task("update", task_id=..., status="review")`
...
## Step 7: Finalize Tasks in Cortex

After successful validation:

1. For each task that has corresponding unit test coverage:
   - Move from "review" to "done" status: `mcp__cortex__manage_task("update", task_id=..., status="done")`
...
1. **NEVER** skip Cortex task management at any point
2. **ALWAYS** create all tasks in Cortex before starting implementation
3. **MAINTAIN** one task in "doing" status at a time
4. **VALIDATE** all work before marking tasks as "done"
5. **TRACK** progress continuously through Cortex status updates
6. **ANALYZE** the codebase thoroughly before implementation
7. **TEST** everything before final completion

## Error Handling

If at any point Cortex operations fail:
1. Retry the operation
2. If persistent failures, document the issue but continue tracking locally
3. Never abandon the Cortex integration - find workarounds if needed

Remember: The success of this execution depends on maintaining systematic task management through Cortex throughout the entire process. This ensures accountability, progress tracking, and quality delivery.