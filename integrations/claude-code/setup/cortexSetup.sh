...
if [ "$CORTEX_API_URL" = "{{CORTEX_API_URL}}" ]; then
  ...
  CORTEX_API_URL="http://${CORTEX_HOST}:8181"
  CORTEX_MCP_URL="http://${CORTEX_HOST}:8051"
  printf "\n  Using API: %s\n  Using MCP: %s\n\n" "$CORTEX_API_URL" "$CORTEX_MCP_URL"
fi
...
print_header() {
  echo
  printf "%s%sCortex Setup%s\n" "$C_BOLD" "$C_CYAN" "$C_RESET"
  printf "  %sServer:%s %s\n" "$C_DIM" "$C_RESET" "$CORTEX_MCP_URL"
  printf "  %sAPI:%s    %s\n" "$C_DIM" "$C_RESET" "$CORTEX_API_URL"
  printf "  %s%s%s\n\n" "$C_DIM" "--------------------------------------------------" "$C_RESET"
}
...
printf "  %sCortex server URLs:%s\n" "$C_BOLD" "$C_RESET"
printf "  API: %s\n" "$CORTEX_API_URL"
printf "  MCP: %s\n" "$CORTEX_MCP_URL"
echo
NEW_API=$(ask "API URL (Enter to accept)" "$CORTEX_API_URL")
if [ "$NEW_API" != "$CORTEX_API_URL" ]; then
  CORTEX_API_URL="$NEW_API"
  # Derive a sensible MCP default by swapping :8181→:8051, then let user confirm
  CORTEX_MCP_URL=$(printf "%s" "$CORTEX_API_URL" | sed 's/:8181/:8051/')
fi
CORTEX_MCP_URL=$(ask "MCP URL (Enter to accept)" "$CORTEX_MCP_URL")
API_BASE="$CORTEX_API_URL"
echo
...
MCP_URL="$CORTEX_MCP_URL/mcp"
ui_info "Configuring MCP endpoint: $MCP_URL"

# Remove any existing cortex MCP from all scopes so the URL is always up to date
claude mcp remove cortex -s local   >/dev/null 2>&1 || true
...
if [ -d "$HOME/.claude/plugins/cache/thedotmack/claude-mem" ] || [ -d ".claude/plugins/claude-mem" ]; then
  ...
  echo "  The cortex-memory plugin replaces claude-mem with Cortex integration."
  echo
...
PLUGIN_DIR="$INSTALL_DIR/plugins/cortex-memory"
...
if [ "$SKIP_PLUGIN_INSTALL" = "false" ]; then
  ...
  if curl -sf "${CORTEX_MCP_URL}/cortex-setup/plugin/cortex-memory.tar.gz" | \
    ...
    ui_warn "Plugin download failed. Install manually from Cortex."
  fi
  echo
fi
...
if [ "$SKIP_PLUGIN_INSTALL" = "false" ]; then
  ...
  if [ "$INSTALL_SCOPE_LABEL" = "project" ]; then
    ...
    LIFECYCLE_PYTHON='$CLAUDE_PROJECT_DIR/.claude/plugins/cortex-memory/.venv/bin/python'
    LIFECYCLE_SCRIPTS='$CLAUDE_PROJECT_DIR/.claude/plugins/cortex-memory/scripts'
    PTU_PYTHON=".claude/plugins/cortex-memory/.venv/bin/python"
    PTU_SCRIPTS=".claude/plugins/cortex-memory/scripts"
  else
  ...
  # Helper: merge cortex hooks into a settings file
  merge_hooks() {
    local target_file="$1"
    shift
...
while i < len(pairs):
    ...
    cleaned = [h for h in existing if not any("cortex-memory" in hk.get("command", "") for hk in h.get("hooks", []))]
    cleaned.append({
        "matcher": "",
...
if curl -sf "${CORTEX_MCP_URL}/cortex-setup/extensions.tar.gz" | \
  ...
  ui_warn "Extension download failed. /cortex-setup will handle installation."
fi
echo
...
CONFIGEOF
ui_success "Wrote $INSTALL_DIR/cortex-config.json"
echo

# ── Update .gitignore ────────────────────────────────────────────────────────

for entry in ".claude/plugins/" ".claude/skills/" ".claude/cortex-config.json" ".claude/cortex-state.json" ".claude/cortex-memory-buffer.jsonl" ".claude/settings.local.json" ".mcp.json" ".cortex/"; do
  grep -qxF "$entry" .gitignore 2>/dev/null || echo "$entry" >> .gitignore
done
ui_success "Updated .gitignore with Cortex local paths."
echo

# ── Inject Cortex rules into CLAUDE.md ──────────────────────────────────────

CLAUDE_MD="CLAUDE.md"
MARKER_START="<!-- cortex-rules-start -->"
...
if [ -n "$SNIPPET" ]; then
  if [ ! -f "$CLAUDE_MD" ]; then
    ...
    ui_success "Created CLAUDE.md with Cortex rules"
  elif grep -qF "$MARKER_START" "$CLAUDE_MD"; then
    # Cortex section already present — replace between markers with latest snippet
...
with open(path, "w") as f:
    f.write(updated)
PYEOF
    ui_success "Updated Cortex rules in CLAUDE.md"
  else
    # CLAUDE.md exists without Cortex section — append with markers
    {
      printf "\n\n%s\n" "$MARKER_START"
      printf "%s\n" "$SNIPPET"
      printf "%s\n" "$MARKER_END"
    } >> "$CLAUDE_MD"
    ui_success "Appended Cortex rules to CLAUDE.md"
    ui_info "Run /cortex-setup in Claude Code for intelligent merge with existing rules."
  fi
else
  ui_warn "Could not download CLAUDE.md snippet. Skipping rules injection."
  ui_info "Run /cortex-setup in Claude Code to configure project rules."
fi
echo
