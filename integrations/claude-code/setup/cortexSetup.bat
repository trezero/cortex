...
set "CORTEX_API_URL={{CORTEX_API_URL}}"
set "CORTEX_MCP_URL={{CORTEX_MCP_URL}}"

echo.
echo  =============================================
echo    Cortex Setup
echo    Server: %CORTEX_MCP_URL%
echo  =============================================
echo.
...
if "!MATCH_COUNT!"=="1" (
  ...
  echo       Matched in Cortex: !MATCHED_TITLE!
...
set "NAME_FILE=%TEMP%\cortex_name.txt"
set "DESC_FILE=%TEMP%\cortex_desc.txt"
set "BODY_FILE=%TEMP%\cortex_body.json"
...
set "PLUGIN_DIR=!INSTALL_DIR!\plugins\cortex-memory"
...
if %errorlevel%==0 (
    ...
    echo       ^! Plugin download failed -- install manually from Cortex
)
...
if "!INSTALL_SCOPE!"=="2" (
    ...
    set "LC_PYTHON=$CLAUDE_PROJECT_DIR/.claude/plugins/cortex-memory/.venv/Scripts/python"
    set "LC_SCRIPTS=$CLAUDE_PROJECT_DIR/.claude/plugins/cortex-memory/scripts"
    set "PTU_PYTHON=.claude/plugins/cortex-memory/.venv/Scripts/python"
    set "PTU_SCRIPTS=.claude/plugins/cortex-memory/scripts"
...
set "LCPY_FILE=%TEMP%\cortex_lcpy.txt"
set "LCSC_FILE=%TEMP%\cortex_lcsc.txt"
...
set "PTUPY_FILE=%TEMP%\cortex_ptupy.txt"
set "PTUSC_FILE=%TEMP%\cortex_ptusc.txt"
...
if %errorlevel%==0 (
    ...
    echo       ^! Extension download failed -- /cortex-setup will handle installation
)
del "%EXT_TMP%" 2>nul
echo.

:: -- Write cortex-config.json -----------------------------------------------
set "FINGERPRINT_FILE=%TEMP%\cortex_fp.txt"
powershell -Command ^
...
set "CONFIG_FILE=%TEMP%\cortex_config_vals.txt"
(echo(!CORTEX_API_URL!)>"%TEMP%\cortex_apiurl.txt"
(echo(!CORTEX_MCP_URL!)>"%TEMP%\cortex_mcpurl.txt"
(echo(!PROJECT_ID!)>"%TEMP%\cortex_pid.txt"
(echo(!PROJECT_TITLE!)>"%TEMP%\cortex_ptitle.txt"
(echo(!MACHINE_FINGERPRINT!)>"%TEMP%\cortex_mfp.txt"
(echo(!INSTALL_SCOPE!)>"%TEMP%\cortex_scope.txt"

powershell -Command ^
  "$apiUrl  = (Get-Content '%TEMP%\cortex_apiurl.txt').Trim(); " ^
  "$mcpUrl  = (Get-Content '%TEMP%\cortex_mcpurl.txt').Trim(); " ^
  "$projId  = (Get-Content '%TEMP%\cortex_pid.txt').Trim(); " ^
  "$projTitle = (Get-Content '%TEMP%\cortex_ptitle.txt').Trim(); " ^
  "$mfp     = (Get-Content '%TEMP%\cortex_mfp.txt').Trim(); " ^
  "$scope   = (Get-Content '%TEMP%\cortex_scope.txt').Trim(); " ^
  "$ts      = (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ'); " ^
  "$cfg = [ordered]@{ cortex_api_url=$apiUrl; cortex_mcp_url=$mcpUrl; project_id=$projId; project_title=$projTitle; machine_id=$mfp; install_scope=$scope; installed_at=$ts }; " ^
  "$cfg | ConvertTo-Json | Set-Content '!INSTALL_DIR!\cortex-config.json'"

echo       ^✓ Wrote !INSTALL_DIR!\cortex-config.json
echo.

:: -- Update .gitignore ------------------------------------------------------
for %%G in (".claude/plugins/" ".claude/skills/" ".claude/cortex-config.json" ".claude/cortex-state.json" ".claude/cortex-memory-buffer.jsonl" ".cortex/") do (
...
if %errorlevel%==0 if exist "%SNIPPET_FILE%" (
    ...
    set "MARKER_END=<!-- cortex-rules-end -->"

    if not exist "CLAUDE.md" (
        :: No CLAUDE.md — create with Cortex rules
        powershell -Command ^
            "$ms = '<!-- cortex-rules-start -->'; " ^
            "$me = '<!-- cortex-rules-end -->'; " ^
        ...
        echo       ^✓ Created CLAUDE.md with Cortex rules
    ) else (
        :: CLAUDE.md exists — check if markers already present
        findstr /c:"<!-- cortex-rules-start -->" "CLAUDE.md" >nul 2>&1
        if !errorlevel!==0 (
            ...
            echo       ^✓ Updated Cortex rules in CLAUDE.md
        ) else (
            ...
            echo       ^✓ Appended Cortex rules to CLAUDE.md
            echo         Run /cortex-setup in Claude Code for intelligent merge with existing rules.
        )
    )
) else (
    echo       ^! Could not download CLAUDE.md snippet. Skipping rules injection.
    echo         Run /cortex-setup in Claude Code to configure project rules.
)
...
set "SYSNAME_FILE=%TEMP%\cortex_sysname.txt"
set "PROJID_FILE=%TEMP%\cortex_projid.txt"
...
powershell -Command ^
  ...
  "if ($projId) { $state | Add-Member -Force -NotePropertyName 'cortex_project_id' -NotePropertyValue $projId }; " ^
