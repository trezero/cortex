#!/usr/bin/env bash
#
# provision-machine.sh — grant a remote machine secure, headless access to Cortex
# at https://cortex.persalto.io via a per-machine Cloudflare Access service token.
#
# What it does (all headless, no browser):
#   1. Mints a new Cloudflare Access service token named "cortex-<machine>".
#   2. Adds that token to the "approved-machines" service-auth policy on the Cortex
...
#   --url <url>          Cortex base URL (default: https://cortex.persalto.io).
...
#   --app-id <id>        Access application id (default: $CORTEX_ACCESS_APP_ID or built-in).
#
set -euo pipefail
...
MACHINE=""; SSH_TARGET=""; CORTEX_URL="$CORTEX_URL_DEFAULT"; APP_ID="$APP_ID_DEFAULT"
...
policy_remove_token() {
  ...
  if [[ "$(echo "$remaining" | jq 'length')" -eq 0 ]]; then
    die "refusing to remove the last token from policy '$POLICY_NAME' (Access requires >=1). Provision a replacement machine first, or delete the Cortex Access app entirely."
  fi
  local ok
...
# Cortex Cloudflare Access credentials for machine: $MACHINE
...
if [[ "$PRINT_ONLY" -eq 1 || -z "$SSH_TARGET" ]]; then
  echo
  echo "=== credential file (place at ~/.config/cortex/cf-access.env, chmod 600) ==="
  echo "$CRED_CONTENT"
  echo
  ...
  echo "  source ~/.config/cortex/cf-access.env"
  echo "  curl -s -H \"CF-Access-Client-Id: \$CF_ACCESS_CLIENT_ID\" -H \"CF-Access-Client-Secret: \$CF_ACCESS_CLIENT_SECRET\" \$CORTEX_URL/api/health"
  exit 0
fi
...
ssh "$SSH_TARGET" "umask 077; mkdir -p ~/.config/cortex" \
  || die "ssh to $SSH_TARGET failed"
printf '%s\n' "$CRED_CONTENT" | ssh "$SSH_TARGET" "umask 077; cat > ~/.config/cortex/cf-access.env"
printf '%s\n' "$MCP_JSON"     | ssh "$SSH_TARGET" "umask 077; cat > ~/.config/cortex/cortex.mcp.json"
echo "Delivered:"
echo "  ~/.config/cortex/cf-access.env       (credential, chmod 600)"
echo "  ~/.config/cortex/cortex.mcp.json     (MCP server config for Claude Code / Cursor)"
echo
echo "On $SSH_TARGET, wire the MCP server into your client, e.g. for Claude Code in a project:"
echo "  cp ~/.config/cortex/cortex.mcp.json <project>/.mcp.json"
echo "Verify connectivity:"
echo "  ssh $SSH_TARGET 'source ~/.config/cortex/cf-access.env && curl -s -o /dev/null -w \"%{http_code}\\n\" \\"
echo "    -H \"CF-Access-Client-Id: \$CF_ACCESS_CLIENT_ID\" -H \"CF-Access-Client-Secret: \$CF_ACCESS_CLIENT_SECRET\" \$CORTEX_URL/api/health'  # expect 200"
