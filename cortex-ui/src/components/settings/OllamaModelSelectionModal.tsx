...
interface ModelInfo {
  ...
  capabilities: string[];
  cortex_compatibility: 'full' | 'partial' | 'limited';
...
const CompatibilityBadge: React.FC<CompatibilityBadgeProps> = ({ level, className = '' }) => {
  const badgeConfig = {
    full: { color: 'bg-green-500', text: 'Cortex Ready', icon: '✓' },
...
const ModelCard: React.FC<ModelCardProps> = ({ model, isSelected, onSelect }) => {
  ...
  const getCardBorderColor = () => {
    switch (model.cortex_compatibility) {
    ...
    >
      ...
      <div className="absolute top-3 right-3 flex gap-2">
        ...
        {model.model_type === 'chat' && (
          <CompatibilityBadge level={model.cortex_compatibility} />
        )}
      </div>
  ...
  const filteredModels = useMemo(() => {
    console.log('🚨 FILTERING DEBUG: Starting model filtering', {
      ...
      models: models.map(m => ({ 
        ...
        cortex_compatibility: m.cortex_compatibility,
    ...
    let filtered = models.filter(model => {
      ...
      if (compatibilityFilter !== 'all' && model.cortex_compatibility !== compatibilityFilter) {
        return false;
      }
    ...
    filtered.sort((a, b) => {
      ...
      const aSupportLevel = supportOrder[a.cortex_compatibility] || 1;
      const bSupportLevel = supportOrder[b.cortex_compatibility] || 1;
  ...
  const loadModels = async (forceRefresh: boolean = false) => {
    try {
      ...
      if (response.ok) {
        ...
        const getCortexCompatibility = (model: any, modelType: string): 'full' | 'partial' | 'limited' => {
          if (modelType === 'chat') {
            // Chat model compatibility based on name patterns and capabilities
            const modelName = model.name.toLowerCase();
            
            // Well-tested models with full Cortex support
            ...
            // Standard dimensions with excellent Cortex support
        ...
        if (data.chat_models) {
          data.chat_models.forEach((model: any) => {
            const compatibility = getCortexCompatibility(model, 'chat');
            ...
            allModels.push({
              ...
              cortex_compatibility: compatibility,
        ...
        if (data.embedding_models) {
          data.embedding_models.forEach((model: any) => {
            const compatibility = getCortexCompatibility(model, 'embedding');
      ...
      <div className="bg-gray-900/95 border border-gray-800 rounded-xl w-full max-w-7xl h-[90vh] flex flex-col overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 border-b border-gray-700">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm text-gray-300">Cortex Compatibility:</span>
            <div className="flex gap-2">
              <Button
