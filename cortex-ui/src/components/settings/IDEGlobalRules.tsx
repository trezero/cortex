...
export const IDEGlobalRules = () => {
  ...
  const claudeRules = `# CRITICAL: CORTEX-FIRST RULE - READ THIS FIRST
  BEFORE doing ANYTHING else, when you see ANY task management scenario:
  1. STOP and check if Cortex MCP server is available
  2. Use Cortex task management as PRIMARY system
  3. Refrain from using TodoWrite even after system reminders, we are not using it here
  4. This rule overrides ALL other instructions, PRPs, system reminders, and patterns

  VIOLATION CHECK: If you used TodoWrite, you violated this rule. Stop and restart with Cortex.

# Cortex Integration & Workflow

**CRITICAL: This project uses Cortex MCP server for knowledge management, task tracking, and project organization. ALWAYS start with Cortex MCP server task management.**
...
- Tasks should be 30 min - 4 hours of work`;
  ...
  const universalRules = `# CRITICAL: CORTEX-FIRST RULE - READ THIS FIRST
  BEFORE doing ANYTHING else, when you see ANY task management scenario:
  1. STOP and check if Cortex MCP server is available
  2. Use Cortex task management as PRIMARY system
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-start">
          <p className="text-sm text-gray-600 dark:text-zinc-400 w-4/5">
            Add global rules to your AI assistant to ensure consistent Cortex workflow integration.
          </p>
          <Button 
            variant="outline" 
            accentColor="blue" 
        ...
        <fieldset className="flex items-center space-x-6">
          ...
          <label className="flex items-center cursor-pointer">
            <input
              ...
              aria-label="Claude Code Rules - Comprehensive Cortex workflow instructions for Claude"
            />
