...
export default defineConfig(({ mode }: ConfigEnv): UserConfig => {
  ...
  // For external access, use CORTEX_HOST (the externally-reachable address)
  const isDocker = process.env.DOCKER_ENV === 'true' || existsSync('/.dockerenv');
  const internalHost = 'cortex-server';  // Docker service name for internal communication
  const externalHost = process.env.CORTEX_HOST || env.CORTEX_HOST || 'localhost';
  ...
  const port = process.env.CORTEX_SERVER_PORT || env.CORTEX_SERVER_PORT || '8181';
  const agentsPort = process.env.CORTEX_AGENTS_PORT || env.CORTEX_AGENTS_PORT || '8052';
  ...
  return {
    ...
    server: {
      ...
      port: parseInt(process.env.CORTEX_UI_PORT || env.CORTEX_UI_PORT || '3737'), // Use configurable port
      ...
      proxy: (() => {
        ...
        if (agentWorkOrdersEnabled) {
          proxyConfig['/api/agent-work-orders'] = {
            target: isDocker ? `http://cortex-agent-work-orders:${agentWorkOrdersPort}` : `http://${externalHost}:${agentWorkOrdersPort}`,
          changeOrigin: true,
          secure: false,
            timeout: 10000, // 10 second timeout
            configure: (proxy: any, options: any) => {
              const targetUrl = isDocker ? `http://cortex-agent-work-orders:${agentWorkOrdersPort}` : `http://${externalHost}:${agentWorkOrdersPort}`;
        ...
        proxyConfig['/agents/health'] = {
          target: isDocker ? `http://cortex-agents:${agentsPort}` : `http://${externalHost}:${agentsPort}`,
          changeOrigin: true,
          rewrite: () => '/health',
        };
        proxyConfig['/agents'] = {
          ...
          configure: (proxy: any, _options: any) => {
            const targetUrl = isDocker ? `http://cortex-agents:${agentsPort}` : `http://${externalHost}:${agentsPort}`;
        ...
        // Cortex setup script download proxy (served by MCP server)
        const mcpPort = env.CORTEX_MCP_PORT || '8051';
        proxyConfig['/cortex-setup'] = {
          target: isDocker ? `http://cortex-mcp:${mcpPort}` : `http://${externalHost}:${mcpPort}`,
          changeOrigin: true,
          xfwd: true,
        };
    ...
    define: {
      ...
      // The browser can't resolve 'cortex-server'
